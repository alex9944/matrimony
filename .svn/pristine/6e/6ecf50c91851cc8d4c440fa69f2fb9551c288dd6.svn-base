...
//$sql = "SELECT MatriID, Status,date_format(Memshipexpirydate,'%d-%M-%Y') as Memshipexpirydate, DATEDIFF( CURRENT_DATE, Memshipexpirydate ) AS expdays  FROM register WHERE DATEDIFF( CURRENT_DATE, Memshipexpirydate ) < 1 LIMIT 0 , 30 ";

$sql = "SELECT MatriID, Status,date_format(expdays,'%d-%M-%Y') as Memshipexpirydate, DATEDIFF( Memshipexpirydate, CURRENT_DATE ) AS expdays FROM register WHERE DATEDIFF( Memshipexpirydate, CURRENT_DATE ) >= -30  order by ID desc ";

//$sql = "select MatriID, Status,date_format(expdays,'%d-%M-%Y') as Memshipexpirydate, DATEDIFF( r.expdays, NOW()) AS expdays from register r where (DATEDIFF( r.expdays, NOW()) > 1  )  ";
