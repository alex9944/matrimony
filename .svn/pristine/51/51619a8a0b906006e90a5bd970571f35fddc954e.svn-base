...
$post_query=mysql_query("select * from class_2daydiscount where  d_status='1'");
      ...
      <div class="element-box">
        ...
        <div class="m" style="padding:14px;">
          <div class="in_head">
            <div class="in_head">
              <div class="in_head">
                <?php $select="select * from class_2daydiscount where d_status='1' "; 
			$strget=$sname1;
			$rowsPerPage = 3;
			...
			$countclass=mysql_num_rows(mysql_query($select));?>
                <div class="head_txt">2Day Discounts<font>
          ...
          <?php if($countclass > 0)
			...
			{
			$dateexp=explode("-",$postsql['d_date']);
			$datesql=mysql_fetch_array(mysql_query("select *,DATE_FORMAT(d_date,'%W %M ') as datechk from class_2daydiscount"));
			$categorysql=mysql_fetch_array(mysql_query("select * from class_category where c_id='$postsql[d_category]'"));
			$subcategorysql=mysql_fetch_array(mysql_query("select * from class_subcategory where sid='$postsql[d_subcategory]'"));
			$sid=base64_encode($postsql['d_id']);
	 $vname=str_replace(" ","-",$postsql['d_title']." ".$sid);
			?>
          <div class="bor_bot " >
            <?php if($postsql['d_photo1'] != '') { 
			
			?>
            <a href="discount_view.php?/<?php echo $vname;?>"><img width="90" height="90" src="admin/<?php echo $postsql['d_photo1']; ?>" alt="" /></a>
            <?php } else { ?>
            <img src="images/img_not.jpg" alt="" />
            <?php } ?>
            <div class="rgtfirst"> <a href="discount_view.php?/<?php echo $vname;?>" class="sub_title"><?php echo $postsql['d_title']; ?></a> <span><?php echo $subcategorysql['subcategoryname']; ?> - <em><?php echo $datesql['datechk']; ?><?php echo $dateexp[2];?>,&nbsp;<?php echo $dateexp[0];?></em></span> <?php echo $postsql['d_description']; ?> </div>
