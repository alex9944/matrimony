      ...
      <div class="profile_block">
        <div class="header">
          <h3 class="float_left"><a href="#" class="title">
            ...
            ) <?php /*?><? echo ucfirst($row['State']); ?> - <? echo ucfirst($row['Country']); ?><?php */?></a></h3>
...
*/	?>
                  ...
                  <tr>
                    <td><img src= "images/photo.jpg" width="15px" height="15px"/>
					  ...
					  { ?>
					    <a href="visiter_notification.php?ID=<?php echo $row['MatriID']; ?>&key=album" title="Register to view Photo Album of <?php echo $row['Name']; ?>" style="padding:2px 0 0 0">Photo album</a> <?php
					  } 
					  else if($_SESSION['status'] == 'Active' or $_SESSION['status'] == 'InActive')
					  { ?>
					    <a href="membershipform.php" title="Subscribe to view Photo Album of <?php echo $row['Name']; ?>" style="padding:2px 0 0 0">Photo album</a> <?php
					  } 
					  else
					  { 
						...
						if($getimagedetailcount > 0)
						 {  ?>
					    <a href="view_photo_album.php?id=<? echo $row['MatriID']; ?>" title="Click here to view Photo Album of <?php echo $row['Name']; ?>" style="padding:2px 0 0 0">Photo album</a> <?php 
						 }
						 ...
						 {  ?>
						   <a href="visiter_notification.php?ID=<? echo $row['MatriID']; ?>&key=album" title="Register to view Photo Album of <?php echo $row['Name']; ?>" style="padding:2px 0 0 0">Photo album</a><?php
						 } 
					  } ?>	
				    </td>
                  </tr>
              ...
              <td align="left" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="profile">
				...
				<?php
				$memcount_det_sql = mysql_query("SELECT Noofcontacts FROM register WHERE MatriID ='".$_SESSION['username']."' ");
				$memcount_det_res = mysql_fetch_array($memcount_det_sql);
				if(isset($memcount_det_res['Noofcontacts']) and $memcount_det_res['Noofcontacts'] > 0)
				{  $view_id  =  $row['MatriID']; ?>				
			  	   <a onclick="MM_openBrWindow('view_address.php?id=<?php echo $view_id; ?>','address','width=350,height=300')" href="javascript:;" style="float:left; font-size:11px; margin-left:0;" class="button-small square-orange">View Address and Email</a> <?php
				} ?>
				 ...
				 if(isset($_SESSION['status']) and $_SESSION['status'] != 'Active')
					{
						...
						if($counthoros_fetch["horoscount"] > 0)
						{
							...
							$sqlss    =  "SELECT *, MatriID,Name,date_format(expdays,'%d-%m-%Y') as memexpdate FROM register where MatriID='$strid' ";
							$memchks = mysql_query($sqlss);
							...
							while($rowss = mysql_fetch_array($memchks)){
							   $horoscope_remaining  =  $rowss['horoscope_remaining'];
							} 
							
							//echo $horoscope_remaining; exit;
