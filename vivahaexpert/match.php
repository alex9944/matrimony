...
{
	window.location.href = 'match.php?page='+pageno+linktext;
}
</script>
...
//END PAGING
		...
		$loginmembersql = "SELECT PE_FromAge,PE_ToAge,PE_MotherTongue,PE_Religion,PE_Caste,PE_Education,PE_Countrylivingin, Status FROM register WHERE MatriID='".$_SESSION['matriid']."'";
		$loginmemberexec = mysql_query($loginmembersql);
		$loginmemberfetch = mysql_fetch_array($loginmemberexec);
				
		$searchquery = "SELECT R.* , pd.matriid, pd.spot_light FROM register R LEFT JOIN payment_details pd ON pd.matriid = R.MatriID  WHERE Gender != '".$_SESSION['gender']."' AND R.MatriID NOT IN (SELECT blockmemberid FROM blockmember WHERE memberid = '".$_SESSION['username']."')";
...
<span class="pathway"> <?Php echo $breadcrumbspage; ?> <img alt="" src="images/icons/arrow_breadcrumbs.png">Your Match</span>
</div>
        </li>
