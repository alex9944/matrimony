...
if(isset($_REQUEST['delid'])){
		...
		mysql_query("delete from class_2daydiscount where d_id='$userid_admin'");
	}
	
	if(isset($_REQUEST['inactid']))
   {
   		 $userid_admin=$_REQUEST['inactid'];
		mysql_query("update class_2daydiscount set d_status='0' where d_id='$userid_admin' ");
		
	}
	if(isset($_REQUEST['actid'])){
		  $userid_admin=$_REQUEST['actid'];
		  mysql_query("update class_2daydiscount set d_status='1' where d_id='$userid_admin' ");
	}
if(isset($_REQUEST['confirm_hotlisting']))
{
	...
	$fet_user = mysql_fetch_array(mysql_query("select * from class_2daydiscount where d_id= '".$t_adid[$i]."'"));
	 mysql_query("update class_register set reg_hotlisting='0' where reg_id='$fet_user[user_id]' ");
	mysql_query("update class_2daydiscount set d_hotlisting='0' where user_id='$fet_user[user_id]'");
    }
 header("location:2daydiscounts.php?added");
	...
	$fet_user = mysql_fetch_array(mysql_query("select d_conemail, d_conname from class_2daydiscount where d_id= '".$t_mail[$i]."'"));
	
	$headers  = "MIME-Version: 1.0\r\n";
$headers .= "Content-type: text/html; charset=iso-8859-1\r\n";
$headers .= "From: $siteadmin" . "\r\n";
 $to      = $fet_user['d_conemail'];
 $name      = $fet_user['d_conname'];
...
//echo $message;die();
 ...
 header("location:2daydiscounts.php?success=1");
...
<table width="98%" border="0" cellpadding="3" cellspacing="1" align="center">
  ...
  <tr>
    <td colspan="11"><table width="100%" border="0" cellspacing="0" cellpadding="0">
        ...
        <tr>
          <td colspan="12" align="center"><font color="#006633" style="font-size:14px;"><b>Removed From Hotlisting List</b></font>
            <META HTTP-EQUIV="REFRESH" CONTENT="10;URL=manageclassified.php">
          </td>
        </tr>
        <?php } ?>
        ...
        <tr>
          <td colspan="6"><form name="frmregister" method="post" action="">
              <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center"  bgcolor="#edcfde">
                <tr>
                  <td colspan="3" height="50" valign="middle" class="head_bg"><b>2Day Discount Ads Management</b></td>
				  <td colspan="3" height="50" valign="middle"  class="head_bg"><span class="link1"><a href="postclassified.php">Add New Discount Classifieds</b></a></span></td>
                </tr>
                <tr>
                  <td colspan="12">&nbsp;</td>
                </tr>
  ...
  <tr>
   ...
   $strget=""; 
     $sel_enq = "select * from `class_2daydiscount`  ";
									   $str_qry="";
									   $temp_qry="";
									...
									{	
										if($str_qry!="")
										 ...
										 $temp_qry="and";
										   ...
										   $str_qry.=$temp_qry." `d_date` >= '$starttime' and  `d_date` <= '$endtime' ";
									...
									{	
										if($str_qry!="")
										 ...
										 $temp_qry="and";
										   ...
										   $str_qry.=$temp_qry." `d_date` = '$starttime'  ";
									...
									{	
										if($str_qry!="")
										 ...
										 $temp_qry="and";
										   ...
										   $str_qry.=$temp_qry." `d_date` = '$endtime' ";
									...
									{	
										if($str_qry!="")
										 ...
										 $temp_qry="and";
										   ...
										   $str_qry.=$temp_qry." d_category	 = '". $_REQUEST['cat_id']."' ";
									...
									{	
										if($str_qry!="")
										 ...
										 $temp_qry="and";
										   ...
										   $str_qry.=$temp_qry." d_subcategory	 = '". $_REQUEST['subcat_id']."' ";
									...
									$subcatqry = mysql_query("select * from class_subcategory where subcategoryname like '%$search_text%'");
										...
										if($str_qry!="")
										 ...
										 $temp_qry="and";
										   ...
										   $str_qry.="(d_title like '%$search_text%' or d_city like '%$search_text%'  or d_description like '%$search_text%' or d_conemail like '%$search_text%' or d_conname like '%$search_text%' ) or (d_conno = '".(int)$search_text."' and d_conno != '0') ";
										  
										   if($substrt!=0)
										   {											
											$str_qry.=" or (d_state IN ($substrt)) ";
										   }
										   
										    if($categ!=0)
										   {											
											$str_qry.=" or (d_category IN ($categ)) ";
										   }
										   
										    if($subcat!=0)
										   {											
											$str_qry.=" or (d_subcategory IN ($subcat)) ";
										   }
									...
									{	
										if($str_qry!="")
										 ...
										 $temp_qry="and";
										   ...
										   $str_qry.=$temp_qry." d_status = '". $_REQUEST['status']."' ";
									...
									if($str_qry!='')
									  ...
									  else
									  {
										   ...
										   {
									    	 $select_User = $sel_enq." order by d_id desc";
										 }
									  }
 ...
 // echo $select_User;
  ...
  ?>
          <tr colspan="9" align="left" bgcolor="#FF83C3">
            <td bgcolor="#faf3f7" class="welcome_text" align="center"><input type="checkbox" name="box[]"  id="box[]" value="<?php echo $reg_fetch['d_id']; ?>" />
            </td>
            <td bgcolor="#faf3f7" class="welcome_text" align="center"><?php echo $i;?></td>
            <td bgcolor="#faf3f7" class="welcome_text" nowrap="nowrap"><a href="view2daydiscounts.php?viewid=<?php echo $reg_fetch['d_id'];?>" class="ad_text_color" title="<?php echo $reg_fetch['p_title'];?>"><?php echo substr($reg_fetch['d_title'],0,22)."..";?></a></td>
            <?php 
	$cat_id=$reg_fetch['d_category'];
    ...
    ?>
            ...
            <?php 
	$subcat_id=$reg_fetch['d_subcategory'];
	...
	?>
            ...
            <td bgcolor="#faf3f7" class="welcome_text"><?php echo $reg_fetch['d_city']; //$reg_fetch['p_conno'];?></td>
            <td bgcolor="#faf3f7" class="welcome_text"><?php echo $reg_fetch['d_conemail'];?></td>
            <td bgcolor="#faf3f7" class="welcome_text" align="center" nowrap="nowrap"><?php echo $reg_fetch['d_date'];?></td>
            <td align="center" bgcolor="#faf3f7"><a href="edit2daydiscounts.php?d_id=<?php echo $reg_fetch['d_id'];?>"><img src="images/edit_f2.png" width="25" height="20" border="0" title="Edit"/></a></td>
            <!--<td bgcolor="#faf3f7"><?php echo $reg_fetch['d_date'];?></td>-->
            <td bgcolor="#faf3f7"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td align="center" valign="bottom"><a href="2daydiscounts.php?delid=<?php echo $reg_fetch['d_id'];?>"><img  src="../images/delete_x.jpg"class="button" value="Delete" onclick="if( confirm('Are you sure you want to Delete this Record?') )
						{
            ...
            <!-- <td align="center" bgcolor="#faf3f7" class="ad_text_color"><a href="viewclassified.php?viewid=<?php echo $reg_fetch['d_id'];?>"><img src="images/view_icon.jpg" width="25" height="20" border="0" title="View"/></a></td> -->
            <td bgcolor="#faf3f7" align="center"><?PHP
		  $userid_admin=$reg_fetch['d_id']; 
		  $activate=$reg_fetch['d_status'];
		  if($activate==1)
		  {
		  ?>
              <a href="2daydiscounts.php?inactid=<?php echo $userid_admin;?>" class="ad_text_color" onclick="if( confirm('Are you sure you want to Inactivate this Record?') )
			{
			return true;
			}
			else
			...
			return false; 
              ...
              <?php } if($activate==0){?>
              <a href="2daydiscounts.php?actid=<?Php echo $userid_admin;?>" class="ad_text_color" onclick="if(confirm('Are you sure you want to Activate this Record?') )
								{
								 return true;
								}
								else
  ...
  <tr>
    <td colspan="11"><input type="submit" value="Compose" name="compose" title="Compose Mail" onclick="return submit_chk();" />
     <!-- <input type="submit" value="Remove" name="hotlisting" title="Remove From Hotlisting" onclick="return submit_chks();" />-->
      ...
      <div id="addhotlisting"  style="overflow:visible; width:455px; background:#ffe1f1 ;  display:none; position:absolute; height:130px; top:500px; left:250px; border:10px solid #D26095;">
        <form name="addhotlisting" action="" method="post" >
          <table width="100%" border="0" cellpadding="4" cellspacing="2">
            ...
            <tr>
              <td colspan="4" height="20" align="center" valign="top" style="padding-left:10px; font-size:12px;" class="link1">&nbsp;Are You sure you Want to remove these ad from hotlisting</td>
            </tr>
		...
		print_r($_REQUEST);
		 ...
		 ?>
            <input type="hidden" value="" name="ad_id" id="ad_id" />
            <tr>
