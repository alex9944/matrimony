...
header("location:index.php");
      ...
      if(isset($_REQUEST['add_sub']))
	{
	 ...
	 mysql_query("insert into class_city(stateid,country,city_name) values('$category',$_REQUEST[mcat],'$subcat')");
	 header("location:changecity.php?catid=$_REQUEST[mcat]&sid=$category");
	 }
?>
	...
	<tr>
		<td height="223" colspan="2">
			<table width="77%" align="center" cellspacing="0" style="border:solid 1px #edcfde;">
				<tr>
				  <td height="27" bgcolor="#edcfde" class="link1">Add New City </td>
				</tr>
				<tr>
				  <td align="right">&nbsp;</td>
				</tr>
				<tr><td valign="top">
					...
					<tr class="normal">
						  <td class="inTxtNormal">&nbsp;&nbsp;<strong>State Name</strong> </td>
						  <td align="left"><select name="cat" id="cat" class="combo_box" style="width:200px;">
						  <option value="">Select Category</option>
						  <?PHP 
						  $cat=mysql_query("select * from class_state where countryid=$_REQUEST[mcat]");
						  while($catresult=mysql_fetch_array($cat))
						  {
						  
						  ?>
						  <option value="<?PHP echo $catresult['stateid'];?>" <?php if($_REQUEST['sid']==$catresult['stateid']) { ?> selected="selected" <?php } ?> ><?PHP echo $catresult['state_name']; }?></option>
						  
						    </select>
							<input type="hidden" name="mcat" id="mact" value="<?php echo $_REQUEST['mcat']; ?>" />
						  </td>
					  </tr>
					  <tr>
						<td align="right">&nbsp;</td>
						...
						<tr >
						  <td width="120" class="inTxtNormal">&nbsp;&nbsp;<strong>City Name</strong> </td>
