...
if(isset($_REQUEST['update']))
{
	...
	$code=$_REQUEST['code'];
	$sql=mysql_query("update google_ad set location='$position',source='$code' where ad_id='$_REQUEST[id]'") ;
	if($sql)
	{
		header("location:googleads.php?upd");
	}
...
function val()
{
 if(document.addads.position.value=="")
 {
  alert("Please Enter The position");
  document.addads.position.focus();
  return false;
 }
 if(document.addads.code.value=="")
 {
  alert("Please Enter The Google Ads Code");
  document.addads.code.focus();
  return false;
 }
}  
function editfun()
	...
	<tr>
		<td height="223" colspan="2">
			<table width="77%" align="center" cellspacing="0" style="border:solid 1px #669966;">
				<tr>
				  <td height="27" bgcolor="#edcfde" class="link1">&nbsp;&nbsp;Edit Google Ads </td>
				</tr>
				<tr>
				...
				<tr><td valign="top" bgcolor="#faf3f7">
					...
					<form name="addads" enctype="multipart/form-data" method="post" onsubmit="return val();">
					<table width="600" border="0" align="center" cellspacing="5" style="border:1px solid #D8F1E4;">
					<?php 
				if(isset($_REQUEST['id']))
				{
				  $id=$_REQUEST['id'];
				  $sql=mysql_query("select * from google_ad where ad_id='$id'");
				  $tab=mysql_fetch_array($sql);
				  if($tab)
				  {
				?>
						<tr class="normal" bgcolor="#faf3f7">
						  <td width="30%" class="link1">&nbsp;&nbsp;Add Location</td>
						  <td width="70%">
						    <input name="position" type="text" id="positions" style="width:420px;" value="<?php echo $tab['location']; ?>"/>
						  </td>
						</tr>
						<tr class="normal" bgcolor="#faf3f7">
						  <td width="30%" class="link1">&nbsp;&nbsp;Google Ads Source</td>
						  <td width="70%" align="">
						    <textarea name="code" rows="7" cols="50"><?php echo $tab['source']; ?></textarea> <br /><br />
				...
				<!-- demo		<input type="submit" name="addcat" value="Submit"  onclick="return editfun()"/> -->
				<input type="submit" name="update" class="but_bg" value="Update"/>
