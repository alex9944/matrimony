...
if(isset($_REQUEST['confirm_hotlisting']))
{
	...
	mysql_query("update class_postclassified set p_hotlisting='0' where p_id='$fet_user[p_id]'");
    }
 header("location:managehotlistings.php?added");
...
$check_exp=mysql_query("select * from `class_postclassified` where p_hotlisting=1");

while($exp_fth=mysql_fetch_array($check_exp))
{
/*echo $today1=date("d-m-Y",strtotime("-31 day",time()));
echo $exp_date1=date("d-m-Y",strtotime($exp_fth['p_hotlist_date']));*/

$today=strtotime("-31 day",time());
$exp_date=strtotime($exp_fth['p_hotlist_date']);

if($today>$exp_date)
{
$update_exp=mysql_query("update class_postclassified set p_hotlisting=0 where p_id=$exp_fth[p_id]"); 
}
 ...
 // echo $select_User;
  ...
  ?>
          <tr colspan="9" align="left" bgcolor="#FF83C3">
            ...
            <td bgcolor="#faf3f7" class="welcome_text" nowrap="nowrap"><a href="viewhotlisting.php?viewid=<?php echo $reg_fetch['p_id'];?>" class="ad_text_color" title="<?php echo $reg_fetch['p_title'];?>"><?php echo substr($reg_fetch['p_title'],0,22)."..";?></a></td>
            <?php 
	...
	?>
            ...
            <td bgcolor="#faf3f7"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td align="center" valign="bottom"><a href="managehotlistings.php?delid=<?php echo $reg_fetch['p_id'];?>"><img  src="../images/delete_x.jpg"class="button" value="Delete" onclick="if( confirm('Are you sure you want to Delete this Record?') )
						{
		  ...
		  {
		  ?>
              <a href="managehotlistings.php?inactid=<?php echo $userid_admin;?>" class="ad_text_color" onclick="if( confirm('Are you sure you want to Inactivate this Record?') )
			{
			return true;
			}
			else
			...
			return false; 
              ...
              <?php } if($activate==0){?>
              <a href="managehotlistings.php?actid=<?Php echo $userid_admin;?>" class="ad_text_color" onclick="if(confirm('Are you sure you want to Activate this Record?') )
								{
								 return true;
								}
								else
