...
function updateplan()
 {
     plan      = document.getElementById('plan').selectedIndex;
	 amount    = document.getElementById('amount');
	 contacts  = document.getElementById('contacts');
	 duration  = document.getElementById('duration');
	 
	 amount.selectedIndex = plan;
	 contacts.selectedIndex = plan;
	 duration.selectedIndex = plan;
	 astromatch.selectedIndex = plan;
	 sms.selectedIndex = plan;
...
<body>
<table width="500" height="500" border="0" cellpadding="0" cellspacing="0" class="GreenBox">
  <tr>
    <td>
	<form action="approve_paid_form_confirm.php" method="post">
	...
	<? while($row = mysql_fetch_array($presult)){ ?>
      <tr>
        <td width="3%" >&nbsp;</td>
        <td colspan="2"><div align="center" class="paratext">Approve Active to Paid </div></td>
        </tr>
      <tr>
        <td height="17">&nbsp;</td>
...
echo( '<select name="txtplan" class="formselect" id="plan" onchange="javascript: updateplan();">' ); 
...
echo( '<select name="txtduration" class="formselect" id="duration">' ); 
...
echo( '<select name="txtplancontact" class="formselect" id="contacts">' ); 
...
echo( '<select name="txtplanamount" class="formselect" id="amount">' ); 
...
?>		</td>
	  ...
	  <tr>
		...
		<?
$planamount = mysql_query("SELECT * from membershipplan ");
echo( '<select name="astromatch" class="formselect" id="astromatch">' ); 
while($row = mysql_fetch_array($planamount))
{ 
echo  '<option value='.$row['astromatch_count'].'>'.$row['astromatch_count'].'</option>';
} 
echo '</select>';  
?>		</td>
      </tr>
	  ...
	  <tr>
		...
		<?
$planamount = mysql_query("SELECT * from membershipplan ");
echo( '<select name="sms" class="formselect" id="sms">' ); 
while($row = mysql_fetch_array($planamount))
{ 
echo  '<option value='.$row['sms_count'].'>'.$row['sms_count'].'</option>';
} 
echo '</select>';  
?>		</td>
      </tr>
      ...
      <tr>
        ...
        <td colspan="2"><div align="center">
          <input name="Submit" type="submit" class="formselect" value="Submit" />
        </div></td>
        </tr>
      <tr>
        <td >&nbsp;</td>
