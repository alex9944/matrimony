...
{
	window.location.href = 'id_search_submit.php?page='+pageno+linktext;
}
...
$max_results = 15; 
 ...
 $religion = array("1"=>"Hindu","10"=>"Muslim - Shia","11"=>"Muslim - Sunni","2"=>"Muslim - Others","12"=>"Christian - Catholic","13"=>"Christian - Orthodox","14"=>"Christian - Protestant","3"=>"Christian - Others","4"=>"Sikh","15"=>"Jain - Digambar","16"=>"Jain - Shwetambar","5"=>"Jain - Others","6"=>"Parsi","7"=>"Buddhist","8"=>"Inter-Religion","9"=>"No Religious Belief");
...
$searchcondition = "";
$searchquery = "SELECT * FROM register r WHERE Status <> 'InActive' AND Status <> ''";
$searchquery .= " AND MatriID = '".$Strid."'";

/*if(isset($_SESSION['username']))
{ 
	$count_blocklist_sql = "SELECT COUNT(*) AS blockmemberlist FROM blockmember WHERE blockmemberid = '".$_SESSION['username']."'";
	...
	{
		$searchquery .= " AND MatriID NOT IN (SELECT blockmemberid FROM blockmember WHERE blockmemberid = '".$_SESSION['username']."')";
	}
}
*/
...
{ 
	...
	if($count_blocklist_fetch["blockmemberlist"] != 0)
	{
		$searchquery .= " AND r.MatriID NOT IN (SELECT blockmemberid FROM blockmember WHERE memberid = '".$_SESSION['username']."' and blockmemberid = '".$Strid."')";
		
		$searchquery .= " AND r.MatriID NOT IN (SELECT memberid FROM blockmember WHERE memberid = '".$Strid."' or blockmemberid = '".$_SESSION['username']."')";
	}
}
  ...
  <div id="content_area" class="content-bg">
    ...
    <form action="" method="post" name="searchrsforms" id="searchrsforms" class="col-md-9">
      <div id="inner_right">
        ...
        <br />
        <strong> You have searched for:</strong> <?php echo $Strid; ?>. <br />
...
?>
        ...
        <div class="profile_block <?php if($pre_count > 0) echo 'profile_block_featured'; ?>">
          <div class="header"><a href="viewfullprofile1.php?id=<?php echo $searchresultfetch['MatriID']; ?>" class="title"><?php echo $searchresultfetch["Name"];?> (<?php echo $searchresultfetch["MatriID"];?>)</a> <a href="#" class="icon_acc_type acc_golden" title="<?php echo $sitename; ?> Golden Account Holder"></a>
	...
	?>
            ...
            <a href="viewfullprofile1.php?id=<?php echo $searchresultfetch['MatriID']; ?>" class="icon_acc_type" title="<?php echo $sitename; ?> <?php echo $plan; ?> Account Holder">
            <?php if($searchresultfetch["Status"] != 'Active') { ?>
            <?php echo $sitename; ?> <?php echo ucfirst(strtolower($searchresultfetch["Status"])); } else { echo 'Basic'; } ?></a> </div>
          <?Php if(isset($_SESSION['username'])) { ?>
          <div style="border-bottom-width: 1px; border-bottom-style: solid; border-bottom-color: #f9d6a5;" class="footer"> <a style="border-left-width: 1px; border-left-style: solid; border-left-color: #f9d6a5;" class="btn_services btn_favorite float_right" href="add_to_favorite.php?id=<?php echo $searchresultfetch['MatriID']; ?>" >Add to Favorite</a>
            <? 
			...
			?>
            <a style="border-left-width: 1px; border-left-style: solid; border-left-color: #f9d6a5;" title="<?=$title?>" class="<?=$css?> btn_varify float_right" href="#" rel="htmltooltip" onclick="return openverifywindow();"><?php echo $verifytext; ?></a>
...
?>
                ...
                <td align="left" valign="top"><table width="100%" border="0" cellpadding="0" cellspacing="0" class="profile">
                    <tr>
                      <th width="89">Age</th><td valign="top">:</td>
                      <td width="435"><?php echo $searchresultfetch["Age"];?> yrs</td>
                      <th width="228">Height</th>
                      <td width="322">:
                        <?php  
...
?></td>
                    ...
                    <tr>
                      <th>Religion</th><td valign="top">:</td>
                      <td><?php echo $religion[$searchresultfetch['Religion']]; ?></td>
                      <th>Caste</th>
                      <td>
			...
			do_request(<?php echo $searchresultfetch['Language'] ?>,<?php echo $searchresultfetch['Religion'] ?>, <?php echo $searchresultfetch['Caste'] ?>,'<?php echo $searchresultfetch['MatriID'] ?>');
                    ...
                    <tr>
                      <th valign="top" >Education</th><td valign="top"></td>
                      <td>
					  <div style="float:left;">:</div><div style="float:left; width:220px;"><?php echo $searchresultfetch['Education']; ?></div></td>
                      <th>&nbsp;</th>
                      <td>&nbsp;</td>
                    </tr>
                    <tr>
                      <th>Occupation</th><td>:</td>
                    ...
                    <tr>
                      <th valign="top">Location </th><td valign="top">:</td>
                      <td>
                        <?php if(isset($_SESSION['username']) and $_SESSION['status'] != '' and  $_SESSION['status'] != 'Active') 
						//echo $searchresultfetch['Address'].", "; ?>
