...
{
	window.location.href = 'newmember.php?page='+pageno+linktext;
}
</script>
...
//$searchquery = "SELECT R.* ,  pd.matriid, pd.spot_light FROM register R LEFT JOIN payment_details pd ON pd.matriid = R.MatriID WHERE Status <> 'InActive' AND Status <> '' AND R.MatriID NOT IN (SELECT blockmemberid FROM blockmember WHERE memberid = '".$_SESSION['username']."')";


$searchquery = "SELECT R.* ,  pd.matriid, pd.spot_light FROM register R LEFT JOIN payment_details pd ON pd.matriid = R.MatriID WHERE Status <> 'InActive' AND Status <> '' ";
...
<span class="pathway"> <?Php echo $breadcrumbspage; ?> <img alt="" src="images/icons/arrow_breadcrumbs.png">New Members</span>
</div>
        </li>
...
?>
			 ...
			 <?php /*?><a style="border-left-width: 1px; border-left-style: solid; border-left-color: #f9d6a5;" class="btn_services btn_varify float_right" href="#">Verify Profile</a><?php */?>
...
?></td>
          ...
          <tr>
            <th>Location</th>
            <td>: <?php echo $searchresultfetch['Address']; ?>
