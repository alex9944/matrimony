...
list($width, $height, $type, $attr) = getimagesize($uploaded_file_path);
...

	{
...

		{
...

			{
				...
				$this->new_name = md5(time()).'.'.$ext;
				$this->new_name1 = md5(time()).rand(1,100).'.'.$ext;

                 
                $font_path = "GILSANUB.TTF";
				$font_size = 8;       // in pixcels
				//$water_mark_text_1 = "9";
				$water_mark_text_2 = $siteadmin;
				 
				//echo "<pre>"; print_r($this->_params);
				 
				//echo getcwd().'/'.$this->_upload_dir.$this->new_name.'<br>';
				//echo $this->_params['tmp_name'];
				
				//echo $this->_upload_dir;
				
				//echo getcwd().'/'.$this->_upload_dir.$this->new_name1;
				
				$upload_status   = move_uploaded_file($this->_params['tmp_name'], getcwd().'/'.$this->_upload_dir.$this->new_name1);
				...
				if($upload_status){
					...
					if($this->watermark_text(getcwd().'/'.$this->_upload_dir.$this->new_name1, getcwd().'/'.$this->_upload_dir.$this->new_name))
					 {
						$demo_image = $this->new_name;
						//exit;
					 }	
					else
					 {
	...
	function watermark_text($oldimage_name, $new_image_name){
	   ...
	   // imagettftext($image, $font_size, 0, 30, 190, $black, $font_path, $water_mark_text_1);
		imagettftext($image, $font_size, 0, 10, 135, $blue, $font_path, $water_mark_text_2);
