...
{
  $keystatus  = " and v.status = '".$_REQUEST['key']."' ";
}
$searchquery = "SELECT R.*,V.* FROM photo_request V,register R WHERE V.receiver  = '".$_SESSION['matriid']."' $keystatus AND V.sender = R.MatriID and V.type = 'photo_password'"; 
...
<span class="pathway"> <?Php echo $breadcrumbspage; ?> <img alt="" src="images/icons/arrow_breadcrumbs.png">Photo Password Request Received</span>
</div>
        </li>
...
<form action="" method="post" name="searchrsforms" id="searchrsforms">
 ...
 <!-- BUSINESS PAGE CONTENT STARTS HERE ############################################ -->   
    <a name="top" id="top"></a>
      ...
      <h1>Photo Password Request Received</h1>
...
//$link = "&txtGender=".$_REQUEST['txtGender']."&txtSAge=".$_REQUEST['txtSAge']."&txtEAge=".$_REQUEST['txtEAge']."&religion=".$_REQUEST['religion']."&caste=".$_REQUEST['caste']."&txtphoto=".$_REQUEST['txtphoto'];
$link = "";
...
?>
			...
			<?php 
			$photostatus  = '';
			$recid        = $_SESSION['matriid'];
			$sender_id    = $searchresultfetch['MatriID']; 
			$statussql    = "select * from photo_request where receiver  = '$recid' and sender  = '$sender_id ' ";
			
			$photores     = mysql_query($statussql);
			$photo_count  = mysql_affected_rows();
			if($photo_count > 0)
			 {
			   while($photo_res = mysql_fetch_array($photores))
			    {
				    $photostatus  =  $photo_res['status'];
					$id           =  $photo_res['id'];
				}
			 }
			//echo $photostatus; 
			$reqid  = $searchresultfetch['MatriID'];
			$recid  = $_SESSION['matriid'];
			  
			if($photostatus == 'pending')
			{ ?>
			 <a style="border-left-width: 1px; border-left-style: solid; border-left-color: #f9d6a5;" class="btn_services btn_block float_right" href="#" onClick="MM_openBrWindow('ppsend.php?id=<?php echo $id; ?>&ReqID=<?php echo $reqid; ?>&MatriID=<?php echo $recid; ?>&key=declined','ppreq','width=650,height=550')">Photo Request Declined</a>
			  <a style="border-left-width: 1px; border-left-style: solid; border-left-color: #f9d6a5;" class="btn_services btn_varify float_right" href="#" onClick="MM_openBrWindow('ppsend.php?id=<?php echo $id; ?>&ReqID=<?php echo $reqid; ?>&MatriID=<?php echo $recid; ?>&key=accept','ppreq','width=650,height=550')">Photo Request Accept</a> <?php
			  $photo_status  = "<b style='color:red;'>$photostatus</b>";
			} 
		   else if($photostatus == 'accept')
		    { ?> 
			 <a style="border-left-width: 1px; border-left-style: solid; border-left-color: #f9d6a5;" class="btn_services btn_block float_right" href="#" onClick="MM_openBrWindow('ppsend.php?id=<?php echo $id; ?>&ReqID=<?php echo $reqid; ?>&MatriID=<?php echo $recid; ?>&key=declined','ppreq','width=650,height=550')">Photo Request Declined</a> <?php
			 $photo_status  = "<b style='color:green;'>$photostatus</b>";
			}	
		   else if($photostatus == 'declined')
		    { ?> 
			   <a style="border-left-width: 1px; border-left-style: solid; border-left-color: #f9d6a5;" class="btn_services btn_varify float_right" href="#" onClick="MM_openBrWindow('ppsend.php?id=<?php echo $id; ?>&ReqID=<?php echo $reqid; ?>&MatriID=<?php echo $recid; ?>&key=accept','ppreq','width=650,height=550')">Photo Request Accept</a> <?php
			   $photo_status  = "<b style='color:red;'>$photostatus</b>";
			}	?>	 
			
			<div style="float:left; margin:7px; text-transform:capitalize;"><b style="color:#A50050;">Status :</b> <?php echo ucwords($photo_status); ?></div> 
